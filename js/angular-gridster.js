...
(function(root, factory) {
	...
	.constant('gridsterConfig', {
		columns: 3, // 网格中的列数
		pushing: false, // 是否要把其他的东西赶出去
		floating: false, // 是否会自动将物品浮起来，这样它们就会堆叠起来
		swapping: false, // 是否有物品交换位置，而不是按相同的尺寸向下推
		width: 'auto', // 网格的宽度。“auto”将把网格扩展到它的父容器
		colWidth:198, // 网格列的宽度。“auto”将网格的宽度平均分配到列中
		rowHeight:50, //高度的网格的行。' match '将使它与列宽相同，数值将被解释为像素，' / 2 '是列宽的一半，' * 5 '是列宽的5倍，等等。
		margins: [0,0], // 网格项之间的边距
		outerMargin: true,
		sparse: false, // “true”可以增加对大网格的拖动和调整的性能(例如20x50)
		isMobile: false, // 切换移动视图
		mobileBreakPoint: 600, // 宽度阈值切换移动模式
		mobileModeEnabled: true, // 当屏幕宽度小于mobileBreakPoint时，是否切换移动模式
		minColumns: 1, // 网格可以扩展到的最小列数
		minRows: 1, // 如果网格是空的，最少行数
		maxRows: 100, // 网格中最大行数
		defaultSizeX: 2, // 列中的项的默认宽度
		defaultSizeY: 1, // 在行的默认高度
		minSizeX: 1, // 一个项目的最小列宽度
		maxSizeX: null, // 一个项目的最大列宽度
		minSizeY: 1, //项目的最小行高度
		maxSizeY: null, // 项的最大行高度
		saveGridItemCalculatedHeightInMobile: false, // 网格项高度在移动显示。正确使用计算高度的sizeY给出
		resizable: { // options to pass to resizable handler
			enabled: true,
			handles: ['se']
			//handles: ['s', 'e', 'n', 'w', 'se', 'ne', 'sw', 'nw']
		},
		draggable: { // 传递给可拖动处理程序的选项
			enabled: true,
			scrollSensitivity: 20, // 相对于指针，从视窗边缘到视窗的距离
			scrollSpeed: 15 //当鼠标指针到达滚动敏感距离时，窗口应该滚动的速度
		}
	})
					...
					return function(scope, $elem, attrs, gridster) {
						...
						if(!scope.isView) {
							scope.$watch(function() {
								if(scope.newRowNum){
									scope.newRowNum.rows = gridster.gridHeight;
								}
		...
		function($document, $window, GridsterTouch) {
			function GridsterDraggable($el, scope, gridster, item, itemOptions) {
				...
				function mouseUp(e) {
					...
					if(gridster.gridHeight > scope.xrow.length) {
						var expandRows = gridster.gridHeight-scope.xrow.length;
						var rowadd=new Array(scope.paixu.length);
						var inx=(scope.xrow.length-1);
